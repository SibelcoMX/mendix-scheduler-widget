...
class SchedulerJS extends Component {
    constructor(props){
        ...
        let schedulerData = new SchedulerData(new moment(new Date()).format('YYYY-MM-DD'), ViewTypes.Week, false, false, {
            schedulerWidth: '90%',
    ...
    async setResources(schedulerData) {
        const { mxform } = this.props;
        const context = this.getContext();
        var resources = [];
        try {
            var objs = await this.executeMicroflow(this.props.resourceSource, context, mxform);
            logger.debug("Received " + objs.length + " resources");
            objs.forEach(resource => resources.push({
        ...
        mx.data.get({
            ...
            callback: function(obj) {
                ...
                if (newSlotId !== oldSlotId) {
                    if (newSlotId === '0')
                        obj.removeReferences('TestApp.Task_Resource', [newSlotId]);
                    else
                        obj.addReference('TestApp.Task_Resource', newSlotId);
                }
                // and commit
                mx.data.commit({
                    mxobj: obj,
    ...
    unplan = (schedulerData, event) => {
        if(confirm(`Do you want to unplan this task? {eventId: ${event.id}, eventTitle: ${event.title}}`)) {
            logger.debug(event);
            // Update task
            this.updateTask(event.id, schedulerData.localeMoment(event.start), schedulerData.localeMoment(event.end), event.resourceId, '0');
    ...
    moveEvent = (schedulerData, event, slotId, slotName, start, end) => {
        if(confirm(`Do you want to move the event? {eventId: ${event.id}, eventTitle: ${event.title}, newSlotId: ${slotId}, newSlotName: ${slotName}, newStart: ${start}, newEnd: ${end}`)) {
            ...
            let dayOfWeek = localeMoment(start).weekday();
            let startHour = localeMoment(start).hour();
            if (((this.props.showWeekend.value || (!this.props.showWeekend.value && dayOfWeek >= 1 && dayOfWeek <= 5)) &&
                 startHour >= localeMoment(this.props.workStartTime.value).hour() && startHour <= localeMoment(this.props.workEndTime.value).hour()) ||
